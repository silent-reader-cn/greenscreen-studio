...
function createProjectsRouter(services) {
  ...
  router.get('/:id/clips', (req, res) => {
    try {
      if (!store.getProject(req.params.id)) return res.status(404).json({ error: 'not found' });
      res.json({
        clips: store.listActionClips(req.params.id, {
  ...
  router.post('/:id/clips', (req, res) => {
    try {
      const clip = store.createActionClip(req.params.id, req.body || {});
      emit({
        type: 'clip',
        message: `Created action clip ${clip.name}`,
        projectId: req.params.id,
        dataChanged: true,
        details: { action: 'create_clip', clipId: clip.id, assetId: clip.assetId },
      });
      res.status(201).json(clip);
    } catch (err) {
      const status = ['PROJECT_NOT_FOUND', 'CLIP_SOURCE_ASSET_INVALID', 'CLIP_NAME_REQUIRED', 'CLIP_RANGE_INVALID', 'CLIP_STATUS_INVALID', 'CLIP_STATUS_INITIAL_INVALID'].includes(err.code) ? 400 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });

  router.get('/:id/clips/:clipId', (req, res) => {
    try {
      const bundle = store.getActionClipBundle(req.params.clipId);
      if (!bundle || bundle.clip.projectId !== req.params.id) return res.status(404).json({ error: 'not found' });
      res.json(bundle);
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  router.patch('/:id/clips/:clipId', (req, res) => {
    try {
      const current = store.getActionClip(req.params.clipId);
      if (!current || current.projectId !== req.params.id) return res.status(404).json({ error: 'not found' });
      const clip = store.updateActionClip(req.params.clipId, req.body || {});
      emit({
        type: 'clip',
        message: `Updated action clip ${clip.name}`,
        projectId: req.params.id,
        dataChanged: true,
        details: { action: 'update_clip', clipId: clip.id, version: clip.version },
      });
      res.json(clip);
    } catch (err) {
      const status = ['CLIP_RANGE_INVALID', 'CLIP_RANGE_EXCLUDES_MARKERS', 'CLIP_STATUS_INVALID'].includes(err.code)
        ? 400
        : ['CLIP_STATUS_TRANSITION_INVALID', 'CLIP_REVIEW_LOCKED', 'CLIP_REVIEW_CHECKS_REQUIRED'].includes(err.code) ? 409 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });

  router.delete('/:id/clips/:clipId', (req, res) => {
    try {
      const current = store.getActionClip(req.params.clipId);
      if (!current || current.projectId !== req.params.id) return res.status(404).json({ error: 'not found' });
      store.deleteActionClip(req.params.clipId);
      emit({
        type: 'clip',
        message: `Deleted action clip ${current.name}`,
        projectId: req.params.id,
        dataChanged: true,
        details: { action: 'delete_clip', clipId: current.id },
      });
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ error: err.message });
  ...
  router.post('/:id/clips/:clipId/markers', (req, res) => {
    try {
      const clip = store.getActionClip(req.params.clipId);
      if (!clip || clip.projectId !== req.params.id) return res.status(404).json({ error: 'not found' });
      const marker = store.addActionMarker(req.params.clipId, req.body || {});
      emit({
        type: 'clip',
        message: `Added ${marker.type} marker at frame ${marker.frame}`,
        projectId: req.params.id,
        dataChanged: true,
        details: { action: 'create_marker', clipId: clip.id, markerId: marker.id },
      });
      res.status(201).json(marker);
    } catch (err) {
      const status = ['CLIP_NOT_FOUND', 'MARKER_FRAME_OUTSIDE_CLIP', 'MARKER_TYPE_INVALID'].includes(err.code)
        ? 400
        : err.code === 'CLIP_REVIEW_LOCKED' ? 409 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });

  router.patch('/:id/clips/:clipId/markers/:markerId', (req, res) => {
    try {
      const clip = store.getActionClip(req.params.clipId);
      if (!clip || clip.projectId !== req.params.id) return res.status(404).json({ error: 'not found' });
      const current = store.getActionClipBundle(clip.id)?.markers.find((item) => item.id === req.params.markerId);
      if (!current) return res.status(404).json({ error: 'not found' });
      const marker = store.updateActionMarker(req.params.markerId, req.body || {});
      emit({
        type: 'clip',
        message: `Updated ${marker.type} marker at frame ${marker.frame}`,
        projectId: req.params.id,
        dataChanged: true,
        details: { action: 'update_marker', clipId: clip.id, markerId: marker.id },
      });
      res.json(marker);
    } catch (err) {
      const status = ['MARKER_FRAME_OUTSIDE_CLIP', 'MARKER_TYPE_INVALID'].includes(err.code)
        ? 400
        : err.code === 'CLIP_REVIEW_LOCKED' ? 409 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  ...
  router.delete('/:id/clips/:clipId/markers/:markerId', (req, res) => {
    try {
      const clip = store.getActionClip(req.params.clipId);
      const marker = store.getActionClipBundle(req.params.clipId)?.markers.find((item) => item.id === req.params.markerId);
      if (!clip || clip.projectId !== req.params.id || !marker) return res.status(404).json({ error: 'not found' });
      store.deleteActionMarker(req.params.markerId);
      emit({
        type: 'clip',
        message: `Deleted ${marker.type} marker`,
        projectId: req.params.id,
        dataChanged: true,
        details: { action: 'delete_marker', clipId: clip.id, markerId: marker.id },
      });
      res.json({ ok: true });
    } catch (err) {
      res.status(err.code === 'CLIP_REVIEW_LOCKED' ? 409 : 500).json({ error: err.message, code: err.code });
    }
  });
  ...
  router.post('/:id/tasks', (req, res) => {
    try {
      ...
      const status = ['PROJECT_NOT_FOUND', 'TASK_TITLE_REQUIRED'].includes(err.code)
        ? 400
        : err.code === 'TASK_EXPORT_CLIP_NOT_FOUND' ? 404
          : String(err.code || '').startsWith('TASK_EXPORT_') ? 409 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });

  router.post('/:id/clips/:clipId/export-task', (req, res) => {
    try {
      const task = store.createActionClipExportTask(req.params.id, {
        clipId: req.params.clipId,
        title: req.body?.title,
        description: req.body?.description,
        assignee: req.body?.assignee || 'ai',
        priority: req.body?.priority || 'normal',
        request: req.body?.request || {},
      });
      emit({
        type: 'collab',
        message: `Queued export task ${task.title}`,
        projectId: req.params.id,
        taskId: task.id,
        dataChanged: true,
        details: { action: 'queue_export_task', clipId: req.params.clipId, priority: task.priority },
      });
      res.status(201).json(task);
    } catch (err) {
      const status = err.code === 'PROJECT_NOT_FOUND' ? 400
        : err.code === 'TASK_EXPORT_CLIP_NOT_FOUND' ? 404
          : String(err.code || '').startsWith('TASK_EXPORT_') ? 409 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
...
function createCollabRouter(services) {
  ...
  router.patch('/tasks/:id', (req, res) => {
    try {
      ...
      const status = err.code === 'TASK_EXPORT_PAYLOAD_IMMUTABLE' ? 409 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });
