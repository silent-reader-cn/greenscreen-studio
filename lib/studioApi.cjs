...
function createProjectsRouter(services) {
  ...
  router.post('/:id/clips', (req, res) => {
    try {
      ...
      const status = ['PROJECT_NOT_FOUND', 'CLIP_SOURCE_ASSET_INVALID', 'CLIP_NAME_REQUIRED', 'CLIP_RANGE_INVALID'].includes(err.code) ? 400 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });
  ...
  router.patch('/:id/clips/:clipId', (req, res) => {
    try {
      ...
      const status = ['CLIP_RANGE_INVALID', 'CLIP_RANGE_EXCLUDES_MARKERS'].includes(err.code) ? 400 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });
  ...
  router.post('/:id/clips/:clipId/markers', (req, res) => {
    try {
      ...
      const status = ['CLIP_NOT_FOUND', 'MARKER_FRAME_OUTSIDE_CLIP', 'MARKER_TYPE_INVALID'].includes(err.code) ? 400 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });
  ...
  router.patch('/:id/clips/:clipId/markers/:markerId', (req, res) => {
    try {
      ...
      const status = ['MARKER_FRAME_OUTSIDE_CLIP', 'MARKER_TYPE_INVALID'].includes(err.code) ? 400 : 500;
      res.status(status).json({ error: err.message, code: err.code });
    }
  });
