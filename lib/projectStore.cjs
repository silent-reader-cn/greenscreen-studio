...
const ACTION_CLIP_EXPORT_TASK_TYPE = 'action_clip_export';
...
function isActionClipExportPayload(payload) {
  return payload?.type === ACTION_CLIP_EXPORT_TASK_TYPE;
}

function snapshotActionClip(clip) {
  return {
    id: clip.id,
...
function snapshotActionMarkers(markers = []) {
  return markers.map((marker) => ({
    ...
    type: marker.type,
    label: marker.label || '',
    payload: cloneJson(marker.payload, {}),
...
function createProjectStore(options = {}) {
  ...
  function updateActionClip(id, patch = {}) {
    ...
    if (contentFields.some((field) => patch[field] !== undefined)) assertClipEditable(current);
  ...
  function createActionClipExportTask(projectId, {
    clipId,
    title,
    description = '',
    assignee = 'ai',
    ...
    const bundle = getActionClipBundle(String(clipId || ''));
    if (!bundle || bundle.clip.projectId !== projectId) {
      throw taskError('TASK_EXPORT_CLIP_NOT_FOUND', 'export task clip was not found in the project');
    }
    if (bundle.clip.status !== 'approved') {
      throw taskError('TASK_EXPORT_CLIP_NOT_APPROVED', 'only approved clips may be queued for AI export');
    }
    const payload = {
      type: ACTION_CLIP_EXPORT_TASK_TYPE,
      schemaVersion: 1,
      queuedAt: nowIso(),
      clipId: bundle.clip.id,
      clip: snapshotActionClip(bundle.clip),
      markers: snapshotActionMarkers(bundle.markers),
      request: cloneJson(request, {}),
    };
    return insertTask(projectId, {
      title: String(title || '').trim() || `Export ${bundle.clip.name}`,
      description,
      assignee,
      priority,
      payload,
  ...
  function createTask(projectId, options = {}) {
    if (isActionClipExportPayload(options.payload)) {
      return createActionClipExportTask(projectId, {
        clipId: options.payload.clipId,
        title: options.title,
        description: options.description,
        assignee: options.assignee,
        priority: options.priority,
        request: options.payload.request,
      });
    }
    return insertTask(projectId, options);
  }

  function validateActionClipExportTask(task) {
    if (!isActionClipExportPayload(task?.payload)) return task;
    const payload = task.payload;
    const bundle = getActionClipBundle(payload.clipId);
    if (!bundle || bundle.clip.projectId !== task.projectId) {
      throw taskError('TASK_EXPORT_CLIP_NOT_FOUND', 'export task clip no longer exists in the project');
    }
    if (bundle.clip.status !== 'approved') {
      throw taskError('TASK_EXPORT_CLIP_NOT_APPROVED', 'export task clip is no longer approved');
    }
    if (!jsonEqual(snapshotActionClip(bundle.clip), payload.clip)
      || !jsonEqual(snapshotActionMarkers(bundle.markers), payload.markers)) {
      throw taskError('TASK_EXPORT_SNAPSHOT_STALE', 'export task clip or marker snapshot is stale');
    }
    return task;
  }
  ...
  function updateTask(id, patch = {}) {
    ...
    if (!current) return null;
    if (isActionClipExportPayload(current.payload)
      && patch.payload !== undefined
      && !jsonEqual(patch.payload, current.payload)) {
      throw taskError('TASK_EXPORT_PAYLOAD_IMMUTABLE', 'export task payload is immutable');
    }
    const next = {
  ...
  function claimNextTask({
    ...
    while (true) {
      ...
      if (!row) return null;
      const candidate = mapTask(row);
      try {
        validateActionClipExportTask(candidate);
      } catch (err) {
        if (!String(err.code || '').startsWith('TASK_EXPORT_')) throw err;
        const ts = nowIso();
        db.prepare(`
          UPDATE collab_tasks
          SET status = 'cancelled', result_json = ?, updated_at = ?
          WHERE id = ? AND status = ?
        `).run(toJson({ code: err.code, error: err.message }), ts, row.id, row.status);
        touchProject(candidate.projectId);
        continue;
      }
      const ts = nowIso();
