...
describe('projectStore + mcpRuntime + studio API', () => {
  ...
  it('creates projects with data folders and CRUD works', () => {
    ...
    expect(bundle).not.toHaveProperty('tasks')
    expect(bundle).not.toHaveProperty('messages')
    expect(store.db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name LIKE 'collab_%'").all()).toEqual([])
  })

  it('removes legacy collaboration tables and their history on migration', () => {
    store.db.exec(`
      CREATE TABLE collab_tasks (id TEXT PRIMARY KEY, title TEXT);
      CREATE TABLE collab_messages (id TEXT PRIMARY KEY, body TEXT);
      INSERT INTO collab_tasks (id, title) VALUES ('legacy-task', 'old task');
      INSERT INTO collab_messages (id, body) VALUES ('legacy-message', 'old message');
    `)
    store.close()
  ...
  it('persists action clips, semantic markers, and review state for a source video', () => {
    ...
    const hit = store.addActionMarker(attack.id, { frame: 45, type: 'instant', payload: { damageMultiplier: 1 } })
    ...
    expect(() => store.addActionMarker(attack.id, { frame: 60, type: 'instant' })).toThrow(/inside the clip range/)
  ...
  it('exposes project APIs without collaboration routes', async () => {
    const app = express()
    ...
    const markerRes = await request(app)
      ...
      .send({ frame: 45, type: 'instant', label: 'damage 1', payload: { hitbox: 'slash_a' } })
    expect(markerRes.status).toBe(201)
    expect(markerRes.body.type).toBe('instant')
    ...
    expect((await request(app).post(`/api/projects/${projectId}/clips/${clipId}/export-task`)).status).toBe(404)
    ...
    expect((await request(app).post(`/api/projects/${projectId}/tasks`)).status).toBe(404)
    expect((await request(app).post(`/api/projects/${projectId}/messages`)).status).toBe(404)
    expect((await request(app).post('/api/collab/tasks/claim-next')).status).toBe(404)
